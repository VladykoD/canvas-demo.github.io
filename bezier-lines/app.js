...
class Animation {
   ...
   createControls() {
      for (let i = 0; i < this.controlsNum + config.curverNum; ++i) {
   ...
   updateCurves() {
      ...
      for (let i = 0; i < config.curverNum; ++i) {
   ...
   drawCurve({startX, startY, controlX1, controlY1, controlX2, controlY2, endX, endY, alpha}) {
      this.ctx.strokeStyle = `rgba(175,231,185,${alpha})`;
   ...
   getYPlacementType(type, i) {
      if (type > .6) {
         return this.size.h / config.curverNum * i;
      } else if(type > .4) {
         return this.size.h
      } else if(type > .2) {
         return this.size.cy
